...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initializeVisitorCounter() {function initializeVisitorCounter() {
  const BASE_VALUE = 23798; // 👈 você edita aqui o número inicial

  let totalVisitors = localStorage.getItem('totalVisitorCount');

  if (totalVisitors === null) {
    totalVisitors = BASE_VALUE;
  } else {
    totalVisitors = Number(totalVisitors);

    if (isNaN(totalVisitors)) {
      totalVisitors = BASE_VALUE;
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("mousemove", (e) => {
        ...
        const img = particles[Math.floor(Math.random() * particles.length)];
